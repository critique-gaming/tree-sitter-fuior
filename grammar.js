module.exports = grammar({
  ...
  rules: {
    source_file: ($) => seq(repeat($._statement), optional($.return_statement)),
    ...
    choose_statement: ($) =>
      seq("choose", $._endl, repeat1($.choice), "end", $._endl),
    choice: ($) =>
      seq(
        "*",
        choice(
    ...
    block: ($) =>
      seq($._endl, repeat($._statement), optional($.return_statement)),

    vararg_definition: ($) =>
      seq(
        "...",
        optional(seq(":", field("type", alias($._type_expression, $.arg_type))))
      ),
    ...
    declare_var_statement: ($) =>
      seq(
        ...
        optional(
          seq(":", field("type", alias($._type_expression, $.var_type)))
        ),
    ...
    command_arg: ($) =>
      choice($._literal, $.bare_word, seq("{", $._expression, "}")),
    ...
    unary_type_expression: ($) =>
      prec(10, choice(seq("?", $._type_expression))),
    ...
    unary_expression: ($) =>
      prec(10, choice(seq("-", $._expression), seq("not", $._expression))),
